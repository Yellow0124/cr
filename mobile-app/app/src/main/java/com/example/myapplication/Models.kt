...
// 📅 月曆應援結構
data class WalletCalendarDay(val day: Int, val key: String, val inMonth: Boolean)
...
data class WalletTicket(
    ...
    val price: String, val notes: String, val imagePath: String = ""
)
...
// 💰 智能預算推薦（對齊組員 Android UI 核心）
enum class BudgetPage { Home, Recommend, Evaluate, Records, AddRecord, Saved, Settings }
enum class BudgetStrategy(val apiValue: String, val label: String) { Balanced("balanced", "均衡"), Value("value", "高價值"), Saving("saving", "省預算") }
data class BudgetRecommendResult(val items: List<BudgetRecommendation> = emptyList())
