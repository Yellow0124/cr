...
def export_to_json():
    ...
    try:
        ...
        # 2. Fetch all tickets
        cursor.execute(
            "SELECT event_id, ticket_type, price FROM event_tickets")
        ...
        output_path = os.path.abspath(os.path.join(
            os.path.dirname(__file__), "..", "database_export.json"))

        print(f"Writing data to {output_path}...")
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(output_data, f, ensure_ascii=False,
                      indent=2, default=datetime_serializer)

        print(
            f"Successfully exported {len(output_data)} events to database_export.json")
