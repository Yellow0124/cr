...
def get_wiki_info(artist):
    ...
    try:
        ...
        html = requests.get(url, headers={
                            "User-Agent": "Mozilla/5.0", "Accept-Language": "zh-TW,zh-Hant;q=0.9,zh;q=0.8"}, timeout=15).text
        xml = etree.HTML(html)
...
for card in cards:
    ...
    try:
        name = ''.join(card.xpath(
            './/div[contains(@class,"text-title")]//text()')).strip()
        if not name:
            continue

        style = ''.join(card.xpath(
            './/*[contains(@style,"background-image")][1]/@style'))
        ...
        # fields
        # ======================
        date_raw = ''.join(detail.xpath(
            '//*[@id="buyTicket"]//div[2]//text()'))
        time_raw = ''.join(detail.xpath(
            '//*[@id="buyTicket"]//div[3]//text()'))
        venue_raw = ''.join(detail.xpath(
            '//*[@id="buyTicket"]//div[4]//text()'))

        activity_date = parse_datetime(date_raw, time_raw)
        venue = re.sub(r'\s+', ' ', venue_raw).strip()

        issuance_raw = ''.join(detail.xpath(
            '//*[@id="activityInfo"]//p[5]//span[2]//text()'))
        issuance = None
        ...
        if issuance_raw:
            try:
                ...
                m2 = re.search(
                    r"(\d{4}[.\-/]\d{2}[.\-/]\d{2}).*?(\d{1,2}:\d{2})", clean)
                if m2:
                    d = m2.group(1).replace(".", "-").replace("/", "-")
                    t = m2.group(2)
                    issuance = datetime.strptime(
                        f"{d} {t}", "%Y-%m-%d %H:%M").strftime("%Y-%m-%d %H:%M:%S")
            except:
                issuance = None

        price_raw = ''.join(detail.xpath(
            '//*[@id="activityInfo"]//p[5]//span[1]//text()'))
        ...
        if v:
            ...
            cursor.execute(
                "INSERT INTO venues(name,address) VALUES(%s,%s)", (venue, venue))
            conn.commit()
