...
router.get('/places', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      message: ['PERMISSION_DENIED', 'REQUEST_DENIED'].includes(err.googleStatus)
        ? 'Google 地點查詢授權未通過，目前顯示備用資料；不代表即時車位狀態'
        : ['RESOURCE_EXHAUSTED', 'OVER_QUERY_LIMIT'].includes(err.googleStatus)
          ? 'Google 地點查詢額度已用盡，目前顯示備用資料'
          : 'Google 地點查詢暫時無法使用，目前顯示備用資料',
      items: fallbackItems
    });
  }
});
...
async function searchGooglePlaces({ apiKey, type, lat, lng, radius, keyword }) {
  ...
  try {
    ...
    try {
      ...
      error.googleStatus = newData?.status || legacyError.googleStatus || 'REQUEST_FAILED';
      error.googleMessage = newData?.message || legacyError.googleMessage || legacyError.message || newApiError.message;
      throw error;
    }
  }
}
